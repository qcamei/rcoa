...
class Lunar
{
     ...
     */
    static private function getLunarInfo($index){
    ...
    {
        $year = (integer)$year; $month = (integer)$month; $date = (integer)$date;
 
        $yearData = self::getLunarInfo($year - self::MIN_YEAR);
    ...
    {
        $year = (integer)$year; $cmonth = (integer)$cmonth; $cdate = (integer)$cdate; $dmonth = (integer)$dmonth; $ddate = (integer)$ddate;
    ...
    /**
     * 计算2个日期之间所有日期
     * @param string $start     起始日期 20120111或者2012-01-11
     * @param string $end       结束日期 20120113或者2012-01-13
     * @param string $jonit     返回日期接合 如 "-" 即返回2012-01-13
     * @return array 
     * @author wskeee
     */
    static public function getDaysBetweenSolar2($start,$end,$joint="-")
    {
        $dt_start = strtotime($start);  
        $dt_end = strtotime($end);  
        $result = [];
        while ($dt_start<=$dt_end){  
            $result [] = date(sprintf("Y%sm%sd", $joint,$joint), $dt_start);
